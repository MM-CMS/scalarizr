...
import agent.scalrinit.app
from agent.scalrinit import metadata
...
class UpdClient(util.Server):
    ...
    def do_start(self):
        ...
        try:
            ...
            self._start_api()  # Start API before bootstrap is important for situation, when
    ...
    def _fix_cache_dir_location(self):
        # [ SCALARIZR-2044]
        old_path = r"c:\var\cache\scalr\updclient\pkgmgr"
        new_path = __node__['updclient']['cache_dir']
        if os.path.exists(old_path):
            if os.path.exists(new_path):
                shutil.rmtree(new_path)
            LOG.debug("Moving cache directory to a new location")
            shutil.move(old_path, new_path)
            shutil.rmtree(r"c:\var\cache\scalr")
            try:
                os.rmdir(r"c:\var\cache")
                os.rmdir(r"c:\var")
            except WindowsError:
                LOG.debug("C:/var directory is not empty. We will not remove it after moving cache.")
