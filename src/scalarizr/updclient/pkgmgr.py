...
STORAGE_DIR = __node__['updclient']['cache_dir']
ARCHIVE_SIZE = 2
...
class MsiexecError(Error):
    pass


class WinPackageManagerError(Error):
    pass
...
def system(*args, **kwds):
    kwds['logger'] = LOG
    kwds['close_fds'] = (sys.platform != 'win32')
    ...
    def __init__(self, path):
        ...
        if self.exists:
            try:
                ...
                raise MetadataError("Invalid metadata file: {0}".format(self._metadata_path))
    ...
    def get(self, name, version=None):
        ...
        if version:
            ...
            raise PackageNotFoundError(
                "Package {0} version {1} can't be found in the repository".format(name, version))
        else:
...
class AbstractPackageManager(object):
    ...
    def fetch(self, name, version, deps=None):
        ...
        STORAGE_DIR/
            .index/
                $hash1 (symlinks to the actual folders below)
                $hash2
                $hash3
            $name1/
                $hash1/
        ...
        try:
            ...
            LOG.warn("Scalarizr was unable to clean up cache directory. "
                     "If this error persits it can consume a lot of disk space.",
    ...
    def status(self, name):
        ...
        versions_available = list(
            sorted(map(attrgetter('version'), package_versions), key=LooseVersion))
    ...
    def install(self, hash):
        ...
        if not os.path.exists(packages_path):
            raise PackageNotFoundError(
                "Package hashed as {0} can't be found in the cache directory {1}".format(hash, STORAGE_DIR))
...
class YumManager(AbstractPackageManager):
    ...
    def _get_installed_version(self, name):
        out, err, returncode = system(
            ('rpm', '-q', '--queryformat', '%{VERSION}', name), raise_exc=False)
        version = None if returncode else out
        return version

    def _install(self, packages_path, metadata):
        packages = tuple(metadata.get('packages'))
        our, err, returncode = system(
            ('rpm', '-U', '--nosignature', '--replacepkgs', '--oldpackage') + packages, cwd=packages_path, exc_class=RpmError)
...
class AptManager(AbstractPackageManager):
    ...
    def _parse_packages_metadata(self, metadata):
        def make_package(entry):
            ...
            hash = re.search(
                r"^{0}:(.*)".format(AptManager.HASH_TYPE), entry, re.M).group(1).strip()
    ...
    def _get_installed_version(self, name):
        out, err, returncode = system(('dpkg-query', '-W', '-f=${Version} ${Status}', name),
                                      raise_exc=False)
        if returncode == 0:
...
class WinPackageManager(AbstractPackageManager):
    # Default checksum to use for verifing files
    HASH_TYPE = 'md5'

    def __init__(self, repo_string):
        super(WinPackageManager, self).__init__(repo_string)
        self.install_dir = os.path.join('C:\\opt')
        self._log_file = os.path.join(STORAGE_DIR, 'scalr-repo-log.txt')

    def _set_repo_string(self, repo_string):
        self.baseurl = repo_string

    def _get_repo_metadata(self):
        r = requests_get(posixpath.join(self.baseurl, "index.json"),
                         exc_class=RepositoryError,
                         exc_message="Cannot retrive repo metadata")
        return json.loads(r.text)

    def _parse_repo_metadata(self, metadata):
        self.repo_type = metadata['repository']
        return None, None

    def _get_packages_metadata(self, packages_path, checksum):
        return self._get_repo_metadata()
    ...
    def _parse_packages_metadata(self, metadata):
        def make_package(entry):
            ...
            location = entry['path']
            hash = entry['md5']
            return Package(name, version, location, hash, self.HASH_TYPE)
        return map(make_package, metadata['packages'])

    def _get_installed_version(self, name):
        """
        It's implied that all packages provided by scalr win repo are omnibus-builded.
        So we can always get version from the omnibus Manifest file
        """
        manifest_path = os.path.join(self.install_dir, name, 'current', 'version-manifest.txt')
    ...
    def _install_single(self, package, packages_path):
        system(('msiexec', '/i', package, '/qn', '/norestart', '/l*vx', self._log_file),
               shell=True,
               cwd=packages_path,
               exc_class=MsiexecError,
               raise_exc=True)
        with codecs.open(self._log_file, 'r', encoding='utf16', errors='ignore') as fp:
            for line in iter(fp.readline, ''):
                if line.find(u'PostInstallSequence returned actual error code') != -1:
                    self._uninstall_single(package, packages_path, no_log=True)
                    raise WinPackageManagerError('PostInstallSequence failed. '
                                                 'See %s' % self._log_file)

    def _install(self, packages_path, metadata):
        # Restore system state if we have half-configured packages before install
        packages = tuple(metadata.get('packages'))
        for package in packages:
            self._install_single(package, packages_path)

    def _uninstall_single(self, package, packages_path, no_log=False):
        # C:\Windows\System32\msiexec.exe
        cmd = ('msiexec', '/qn', '/x', package)
        if not no_log:
            cmd += ('/l*', self._log_file)
        system(cmd,
               shell=True,
               cwd=packages_path,
               exc_class=MsiexecError)
    ...
    def uninstall(self, name):
        LOG.info('Uninstalling package %s' % name)
        pkg_status = self.status(name)
        hash_ = pkg_status.get('installed_hash')
        if hash_:
            packages_path = os.path.join(STORAGE_DIR, '.index', hash_)
            if not os.path.exists(packages_path):
                raise PackageNotFoundError("Package installer at {0}"
                                           " can't be found in the cache directory {1}".format(hash_, STORAGE_DIR))
            metadata = Metadata(packages_path)
            for package in tuple(metadata.get('packages')):
                self._uninstall_single(package, packages_path)
        else:
            LOG.info('Package "%s" is not installed' % name)
...
def create_pkgmgr(repo_url):
    if scalarizr.linux.os['family'] in ('RedHat', 'Oracle'):
        return YumManager(repo_url)
    if scalarizr.linux.os['family'] == 'Windows':
        return WinPackageManager(repo_url)
