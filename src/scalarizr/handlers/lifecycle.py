...
class LifeCycleHandler(scalarizr.handlers.Handler):
    ...
    saved_boot_id_file = os.path.join(__node__['private_dir'], 'boot_id')
    ...
    def on_init(self):
        ...
        if __node__['state'] == 'running':
            scalarizr.handlers.check_supported_behaviors()
    ...
    def on_start(self):
        ...
        if os.path.exists(self.saved_boot_id_file):
            ...
            if saved_boot_id and saved_boot_id != current_boot_id \
                ...
                self._logger.debug('Boot-ID changed (new: {}, saved: {})'.format(
        ...
        if Flag.exists(Flag.REBOOT) or Flag.exists(Flag.HALT):
            if Flag.exists(Flag.REBOOT):
                ...
                self._logger.info("Scalarizr resumed after server stop (same as reboot)")
    ...
    def _start_init(self):
        ...
        # Prepare HostInit
        __node__['boot_time'] = time.time() - sysutil.uptime()
        msg = self.new_message(Messages.HOST_INIT,
            dict(
                seconds_since_start=float('%.2f' % (time.time() - __node__['start_time'], )),
                seconds_since_boot=float('%.2f' % (time.time() - __node__['boot_time'], )),
...
class IntMessagingService(object):
    ...
    def __init__(self):
        ...
        self._msg_service = f.new_service("p2p", **{
            'server_id' : cnf.rawini.get(config.SECT_GENERAL, config.OPT_SERVER_ID),
            'crypto_key_path' : cnf.key_path(cnf.FARM_KEY),
            'consumer_url' : 'http://0.0.0.0:8012',
            'msg_handler_enabled' : False
        })
